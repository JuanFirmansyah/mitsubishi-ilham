...
const HERO_SLIDES = [
  {
    id: 1,
    titleTop: "KAMI HADIR",
    titleMain: "UNTUK SULAWESI SELATAN",
    desc: "Gratis Towing, Program Servis & Bodi Cat khusus wilayah Sulawesi Selatan.",
    cta1: "Cari Dealer",
...
export default function Home() {
  ...
  // Navigasi car dengan keyboard & touch
  ...
  // Handle touch events untuk carousel car
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 text-white">
      ...
      {/* Model Showcase - Improved with Touch Support */}
      <section className="relative py-24 md:py-32 bg-white text-black overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 py-16 relative">
            ...
            {CAR_MODELS.map((car, index) => {
              ...
              const getOffset = () => {
                if (typeof window !== 'undefined' && window.innerWidth < 640) {
                  return 200; // Mobile offset
                }
                return 320; // Desktop offset
              };
              ...
              return (
                <div
                  ...
                  className={`absolute transition-all duration-700 ease-out cursor-pointer
                    ...
                    ${offset === -1 ? `-translate-x-[${getOffset()}px] scale-90 opacity-40 z-10` : ""}
                    ${offset === 1 ? `translate-x-[${getOffset()}px] scale-90 opacity-40 z-10` : ""}
                    ${Math.abs(offset) > 1 ? "opacity-0 scale-75" : ""}
                  `}
                  style={{
                    transform: `translateX(${offset * getOffset()}px) scale(${offset === 0 ? 1 : 0.9})`,
                    opacity: Math.abs(offset) > 1 ? 0 : (offset === 0 ? 1 : 0.4),
                    pointerEvents: offset === 0 ? 'auto' : 'none',
                  }}
                >
                  {/* Click to center */}
                  <div onClick={() => setActiveCar(index)}>
                    {/* Badge */}
                    {car.isNew && (
                    ...
                    {/* Car Image */}
                    <div className="w-[200px] sm:w-[280px] md:w-[360px] lg:w-[420px]">
                      <Image
                        src={car.image}
                        alt={car.name}
                        width={420}
          ...
          <button
            onClick={prevCar}
            className="absolute left-0 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center border border-black/30 rounded-full hover:bg-black hover:text-white transition bg-white/80 backdrop-blur-sm"
          ...
          <button
            onClick={nextCar}
            className="absolute right-0 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center border border-black/30 rounded-full hover:bg-black hover:text-white transition bg-white/80 backdrop-blur-sm"
            aria-label="Next car"
          >
            →
          </button>
          ...
          <div className="flex justify-center gap-2 mt-8">
            {CAR_MODELS.map((_, index) => (
              <button
                key={index}
                onClick={() => setActiveCar(index)}
                className={`w-2 h-2 rounded-full transition-all ${
                  index === activeCar ? 'w-8 bg-black' : 'bg-gray-300 hover:bg-gray-400'
                }`}
